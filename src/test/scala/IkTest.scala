...
class IkTest {
  ...
  @Test(enabled = true, priority = 1, expectedExceptions = Array(classOf[IllegalArgumentException]))
  def runTest3() = {
    val output = IkClient.getIkTokens("")
    LOG.info("this is the result={}", output)
  }

  @Test(enabled = true, priority = 1, expectedExceptions = Array(classOf[IllegalArgumentException], classOf[NullPointerException]))
  def runTest4() = {
